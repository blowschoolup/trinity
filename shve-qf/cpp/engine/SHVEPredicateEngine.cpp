...
SHVEPredicateEngine::SHVEPredicateEngine(bool for_encryption, KeyParam* key): PredicateOnlyAESBlockCipher(for_encryption,key) {
    if (for_encryption_) {
...
char** SHVEPredicateEngine::process(char** C, int inOff, int inLen) {
...
char* SHVEPredicateEngine::process_od(char* C, int inOff, int inLen) {
    if (dynamic_cast<SHVESecretKeyParam*>(key_) != nullptr) {   // evaluation
        SHVESecretKeyParam* secretKey = (SHVESecretKeyParam*)key_;
        unsigned char z[16];
        memcpy(z,secretKey->getD0(),16);
